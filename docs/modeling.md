...
**다음 할 일**: Colab GPU에서 `config/default.yaml`(kneeKL299, efficientnet_b3, 25 epoch)로 재학습 → 5차 결과를 4차(224 입력)와 비교. 참고: 해상도가 커진 만큼 GPU 메모리 사용량이 늘어나므로, OOM 발생 시 `train.batch_size`를 32→16~24로 낮출 것
...
**문제**: 프로젝트 폴더(`knee-kl-grade-classification`)가 별도 git 저장소가 아니라, git 루트가 `C:\Users\knh84`(홈 폴더 전체)로 잡혀 있었고 원격도 무관한 다른 저장소(`hyun.git`)를 가리키고 있었음. 이 상태로 커밋하면 Desktop, Documents, AppData, 카카오톡 데이터 등 개인 파일이 통째로 git에 들어갈 위험이 있었음.

**해결**: 프로젝트 폴더 안에 별도로 `git init`해서 독립된 저장소로 분리. 홈 폴더의 기존 `.git`은 건드리지 않고 그대로 둠.

---

## 2. 디스크 용량 확인 오차 (Git Bash vs PowerShell)

**문제**: 데이터셋(7GB zip) 압축 해제 전 `df -h`(Git Bash)로 확인했을 때 C드라이브 여유공간이 1.2GB로 나와서 압축 해제가 불가능해 보였음.

**해결**: PowerShell의 `Get-PSDrive`로 다시 확인하니 실제로는 14.3GB 여유가 있었음 — Git Bash의 `df`가 Windows 환경에서 부정확하게 표시되는 경우가 있어서, 디스크 용량은 PowerShell 기준으로 확인하는 게 안전함.

---

## 3. CPU 학습 속도 문제 → Colab 전환
...
## 4. Device 불일치 버그 (CPU→GPU 전환 시 발생)
...
## 5. Threshold 튜닝 효과가 모델마다 반대로 나타남
...
**문제**: Colab에서 여러 번 재학습하면서 다운로드한 `best.pth`, `best (1).pth`, `best (2).pth`... 파일이 브라우저에서 자동으로 번호가 매겨져 쌓였고, 한 번은 이전 파일을 잘못 보내서 분석에 혼선이 있었음.

**확인 방법**: 파일 크기와 수정 시각을 `ls -la`로 대조해서 실제로 새 파일인지 확인 (예: 같은 모델 구조면 파일 크기가 동일해야 정상, 백본이 바뀌면 크기도 달라짐 — B0는 약 16MB, B3는 약 43MB).

---

## 7. 데이터 누수(Data Leakage) 점검 (문제는 아니었지만 검증한 내용)
